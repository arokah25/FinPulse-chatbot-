...
#Class methods
    ...
    # #initialization method, accept cache directory for possibly already downloaded data
    # def __init__(self, cache_dir: str = "data/cache"):
    #     #convert the cache directory string to path object
    #     self.cache_dir = Path(cache_dir)
    #     #make sure thhat the cache directory exists
    #     self.cache_dir.mkdir(parents=True, exist_ok=True)
    #     #builds path object pointing to data/cache/company_tickers.json; caches ticker->CIK mapping -> future runs can load it quickly
    ...
    def _get_tickers_mapping(self) -> Dict[str, str]:
        ...
        # # if cache file already exists -> load it -> DEACTIVATE IT FOR NOW, cause it falls always back
        # if self.tickers_file.exists():
        #     logger.info("Loading cached ticker mapping") #logs info message
        #     try:
        #         #open file at self.tickers_file for reading
        #         with open(self.tickers_file, 'r') as f: #open JSON for reading
        #             #parse JSON into dictionary
        #             data = json.load(f)
        #             mapping_dictionary = {item['ticker']: item['cik_str'] for item in data.values()}
        #             #return {item['ticker']: item['cik_str'] for item in data.values()}
        #             logger.info("Accessing cache successful")
    ...
    #     try:
    #         with open(self.tickers_file, 'w') as f: #open cache file for writing
    #             #save {ticker:cik} dicitionary to disk
    #             json.dump(common_tickers, f)
    #     except Exception as e:
    #         #logs issue but continues
        ...
        for kpi_name, sec_name in kpi_mappings.items():
            ...
            if sec_name in us_gaap:
                ...
                def get_latest_10q_data(unit_data):
                    ...
                    #list comprehension: builds new list by scanning every item in unit_data
                    #safety tries to read "form" key from the dictioanry; empty string if its missing
                    # q10_data = [item for item in unit_data if item.get('form', '').startswith('10-Q')] #returns false for 10-K
                    #--------------------------------------------
    ...
    #method to fetch the text contents of a fling, inputs: accesion_number, primary_document, optional cik; output: filing text content as string
